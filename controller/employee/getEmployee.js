...
const getAllEmployees = async (req, res) => {
  try {
    ...
    return res.status(200).json(employees.reverse());
  } catch (error) {
...
const getSingleEmpById = async (req, res) => {
  try {
    const empId = req.empId; // logged-in user id
    ...
    if (!["incharge", "manager"].includes(retrievedEmp.designation)) {
      return res.status(403).json({
        success: false,
        message: "Access denied. Only administrators can get employee details.",
      });
    }
    const singleEmployee = await Employee.findById(dbid);
...
module.exports = { getAllEmployees, getSingleEmp , getSingleEmpById};
